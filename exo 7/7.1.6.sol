...
contract ERC721Simple {
    ...
    function transferFrom(address _from, address _to, uint256 _tokenId) public {
        ...
        require(_tokenId/1000 != 2, "Les objets divins ne sont pas transférables.");
...
contract ObjetsMagiques is ERC721Simple {
    ...
    function utiliser(uint _tokenId) public returns (uint nbRandom) {
        require(exists(_tokenId),"Cet objet n'existe pas.");
        require(tokens[_tokenId] == msg.sender, "Vous devez être le propriétaire de l'objet pour l'utiliser.");
        
        uint useItem = (uint(blockhash(block.number-1))%11);
